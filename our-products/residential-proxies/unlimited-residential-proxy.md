---
icon: triple-chevrons-up
---

# Unlimited Residential

<mark style="color:purple;">Unlimited Residential</mark> основан на пуле [Standard Residential](standard-residential.md) - те же устройства, те же страны, тот же диапазон IP. Главное отличие: **без ограничений по трафику**. Тариф выгоден там, где Standard с оплатой за гигабайт становится дорогим.

{% embed url="https://dashboard.proxyshard.com/en/residential-unlimited" %}

Пошаговая инструкция по покупке и оплате: [Приобретение резидентских прокси](../../site-navigation/buying-and-renewing/buying-residential-proxies.md).

## Характеристики

| Параметр                    | Значение                                    |
| --------------------------- | ------------------------------------------- |
| Размер пула                 | 300 000 - 400 000 устройств (= Standard)    |
| Макс. кол-во соединений     | 5 000                                       |
| Макс. скорость на заказ     | 75 Mbps                                     |
| [Поддержка UDP](../about-udp/) | ✓ (кроме США; есть [ограничения по портам](../restrictions.md)) |
| [Фильтрация Device OS](../p0f-spoofing.md) | ✗ |
| Тарификация                 | День / Половина месяца / Месяц              |
| Стоимость                   | **$30** / день · **$199** / нед. · **$399** / полм. · **$699** / мес. |

{% hint style="warning" %}
Лимит в 5 000 соединений распространяется на все типы (ESTAB, FIN\_WAIT и другие). Увеличение доступно на месячном тарифе - напишите в [Support](../../contact-us.md).
{% endhint %}

## Для каких задач подходит

- Автоматизация с высоким потреблением трафика
- Долгосрочные процессы, где важно не считать гигабайты
- Задачи, требующие поддержки <mark style="color:purple;">UDP / WebRTC</mark>

{% hint style="info" %}
Нужно платить за гигабайты, а не по подписке? Смотрите [Standard Residential](standard-residential.md).\
Нужен более крупный пул с тонким таргетингом? Смотрите [Premium Residential](premium-residential.md).
{% endhint %}

С ограничениями продукта можно ознакомиться [тут](../restrictions.md).
